...
class BoardWidget extends Pad  {
        ...
        for (char col = 'a'; col <= 'g'; col++) {
            for (char row = '1'; row <= '7'; row++) {
                if (_model.get(col, row) == RED) {
                    ...
                    g.fillOval((col - 'a') * SQDIM + SQDIM / 2 - PIECE_RADIUS,
                            ('7' - row) * SQDIM + SQDIM / 2 - PIECE_RADIUS,
                            PIECE_RADIUS * 2,
                            PIECE_RADIUS * 2);
                    ...
                    g.setColor(BLOCK_COLOR);
                    int x = (col - 'a') * SQDIM + SQDIM / 2 - BLOCK_WIDTH / 2;
                    int y = ('7' - row) * SQDIM + SQDIM / 2 - BLOCK_WIDTH / 2;
    ...
    void drawBlock(Graphics2D g, int cx, int cy) {
        g.setColor(BLOCK_COLOR);
        int x = cx - BLOCK_WIDTH / 2;
        int y = cy - BLOCK_WIDTH / 2;
        g.fillRect(x, y, BLOCK_WIDTH, BLOCK_WIDTH);
    }
    ...
    private void handleClick(String unused, MouseEvent where) {
        ...
        if (where.getButton() == MouseEvent.BUTTON1) {
            ...
            if (mouseCol >= 'a' && mouseCol <= 'g'
                ...
                if (_blockMode) {
                    String command = "block " + mouseCol + mouseRow;
                    _commandQueue.offer(command);
                } else {
                    if (_selectedCol != 0) {
                        String command = "" + _selectedCol
