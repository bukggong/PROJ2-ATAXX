...
class Board {
        ...
        for (int i = 0; i < SIDE; i++) {
            ...
            for (int j = 0; j < SIDE; j++) {
                ...
                if ((col == 'a' && row == '1')
                        ...
                        && (row == (char) ('1' + SIDE - 1)))) {
                    set(col, row, BLUE);
                } else if (((col == (char) ('a' + SIDE - 1)) && row == '1')
                        || (col == 'a' && (row == (char) ('1' + SIDE - 1)))) {
                    set(col, row, RED);
                }
            }
        }
    ...
    boolean legalMove(Move move) {
        ...
        if (move.isPass()) {
            ...
            if (!(whoseMove() == _board[move.fromIndex()])) {
                return false;
            } else {
        ...
        for (char i = '1'; i <= '7'; i++) {
            for (char j = 'a'; j <= 'g'; j++) {
                if (get(j, i) == who) {
                    for (int l = -2; l <= 2; l++) {
                        for (int k = -2; k <= 2; k++) {
                            char newcha1 = (char) (j + l);
                            char newcha2 = (char) (i + k);
                            if (get(newcha1, newcha2) == EMPTY) {
                                return true;
                            }
                        }
                    }
    ...
    void makeMove(Move move) {
        ...
        PieceColor org = get(move.col0(), move.row0());
        if (move.isExtend()) {
            _numJumps = 0;
            set(move.col1(), move.row1(), org);
        ...
        for (int i = 0; i < 3; i++) {
            char col = (char) (move.col1() - 1 + i);
            for (int j = 0; j < 3; j++) {
                char row = (char) (move.row1() - 1 + j);
                if (get(col, row) == opponent) {
                    set(col, row, _whoseMove);
                    incrPieces(_whoseMove, 1);
        ...
        PieceColor other = turn.opposite();
        if (redPieces() == 0) {
            _winner = BLUE;
            ...
            _winner = RED;
                ...
                && !canMove(turn))
                || numJumps() == JUMP_LIMIT
                || redPieces() + bluePieces() == totalOpen()
    ...
    void undo() {
        while (_undoSquares.peek() != null && _undoPieces.peek() != null) {
            int numUndo = _undoSquares.pop();
            incrPieces(get(numUndo), -1);
            PieceColor color = _undoPieces.pop();
            incrPieces(color, 1);
            unrecordedSet(numUndo, color);
        }
    ...
    boolean legalBlock(char c, char r) {
        if (_board[index(c, r)] == EMPTY && _numJumps == 0 && _numMoves == 0) {
            return true;
        }
        return false;
    }
    ...
    void setBlock(char c, char r) {
        ...
        reB = index(newC, newR);
        _board[index1] = _board[reC] = _board[reR] = _board[reB] = BLOCKED;
    ...
    int totalOpen() {
        int blocked = 0;
        int unblocked = SIDE * SIDE;
        for (int i = 0; i < SIDE; i++) {
            char col = (char) ('a' + i);
            for (int j = 0; j < SIDE; j++) {
                char row = (char) ('1' + j);
                if (_board[index(col, row)] == BLOCKED) {
                    blocked += 1;
                }
            }
        }

        int result = (unblocked - blocked);
